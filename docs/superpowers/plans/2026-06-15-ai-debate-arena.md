...
> **更新（去 fork）**：调研后确认不需要 ChatALL 的代码（我们用纯 DOM 驱动，与其 cred+API 思路不同）。已**删除 ChatALL 全部源码，改为独立 Electron 实现**，整库 **MIT** 授权。Task 1 的"fork"现理解为"调研参考 + 自建工程"；应用外壳从 `spike/`（已验证的纯 Electron demo）演进而来，不再依赖 ChatALL 的 Vue/构建体系。
